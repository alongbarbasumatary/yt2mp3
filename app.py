...
def download_mp3(job_id, url):
    ...
    try:
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            cleanup_file(mp3_path, delay=600)
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  body::before {
    ...
    position: fixed;
    inset: 0;
    background: radial-gradient(ellipse 80% 60% at 50% -10%, #ff4d0015 0%, transparent 70%);
    pointer-events: none;
  }
  .logo {
...
<div class="card">
  ...
  <div class="input-row">
    ...
    <button id="dlBtn" onclick="startDownload()">GRAB</button>
  </div>
...
async function startDownload() {
  ...
  try {
    ...
    setStatus('Network error', 0, true);
    resetBtn();
  }
}
