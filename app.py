...
"""
╔══════════════════════════════════════════════════╗
║   YouTube Downloader Bot - Render Deploy Ready   ║
║   Telegram Bot API + Telethon MTProto            ║
║   Env-vars | String session | Flask keep-alive   ║
╚══════════════════════════════════════════════════╝
"""
...
MY_CHAT_ID   = int(MY_CHAT_ID_S) if MY_CHAT_ID_S else None
...
# Telethon with string session — no .session file needed on Render
        ...
        if MY_CHAT_ID and chat_id != MY_CHAT_ID:
            try:
                notify_caption = (
                    ...
                    f"📺 Format: {label}\n"
                    f"📦 Size: {human_size(fsize)}\n"
                    f"🙍 Requested by: {user.full_name} (`{user.id}`)"
                )
...
# ─── FLASK KEEP-ALIVE (Render requires HTTP server) ──────────────
flask_app = Flask(__name__)

@flask_app.route("/")
def home():
    return "🤖 YouTube Bot is running!", 200
...
async def run():
    ...
    log.info(f"✅ Flask keep-alive running on port {PORT}")

    await tg_client.start(bot_token=BOT_TOKEN)
    log.info("✅ Telethon client ready")

    # Print session string if not set (first deploy helper)
    if not SESSION_STR:
        log.warning("=" * 60)
        log.warning("⚠️  SESSION_STRING not set! Copy this to Render env vars:")
        log.warning(tg_client.session.save())
        log.warning("=" * 60)
    ...
    try:
        ...
        if MY_CHAT_ID:
            try:
                await app.bot.send_message(
                    MY_CHAT_ID, "🔴 *YouTube Bot is offline.*",
                    parse_mode="Markdown"
                )
            except Exception:
                pass
